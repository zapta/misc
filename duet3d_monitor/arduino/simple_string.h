...
class String : public Vector<char, N + 1> {
 public:
  String() : Vector<char, N + 1>() { this->clear(); }

  String(const char* s) : Vector<char, N + 1>() {
    this->clear();
    this->copy(s);
  }
  ...
  const char* c_str() const { return this->storage; }
  ...
  bool equals(const char* s) const { return strcmp(s, this->storage) == 0; }
...
// Redefine 'add' to add a null terminator
template <int N>
bool String<N>::add(const char* p, int n) {
