...
template <class T, int N>
bool Vector<T, N>::add(const T* p, int n) {
  while (n != 0) {
    if (filled == N) {
      return false;
    }
...
template <class T, int N>
void Vector<T, N>::erase(int pos, int count) {
...
template <class T, int N>
void Vector<T, N>::truncate(int pos) {
  if (pos < filled) {
    filled = pos;
  }
}
...
template <class T, int N>
bool Vector<T, N>::replace(T oldVal, T newVal) {
